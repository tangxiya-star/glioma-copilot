...
_FIT_SYSTEM = (
    ...
    "  - \"met\": the patient satisfies this line AND the report actually contains the "
    "basis to know it. For an EXCLUSION, 'met' = the report gives grounds that the patient "
    "does NOT have the excluding condition — e.g. the treatment record shows no prior "
    "bevacizumab (=> met), or the patient is male so a pregnancy/breastfeeding exclusion is "
    "not applicable (=> met). Handle negation carefully.\n"
    "  - \"not_met\": the patient clearly fails/violates this line.\n"
    "  - \"unknown\": the report LACKS the information to decide — whether a test value / "
    "result (labs, serologies HIV/HBV/HCV, EGFR, MRI, washout dates, cardiac LVEF/QTcF) OR "
    "a clinical-history item that simply isn't recorded (drug-allergy history, prior second "
    "malignancy, uncontrolled illness, COVID status, concurrent investigational agents). Do "
    "NOT assume an absence the chart never documents. Say what test or record is needed — "
    "this is the patient's workup checklist.\n\n"
    "Rules: judge ONLY from the report — never assume a fact the chart lacks, whether a "
    "value OR an absence. For every item, quote the exact eligibility line in `citation`. "
    "Be COMPLETE — cover every distinct eligibility criterion (aim for ~20-24 items), NOT "
    "just the molecular ones: include labs, serologies, cardiac, organ/marrow recovery, "
    "washout, concurrent agents, contraception, pregnancy. You may merge several "
    "near-identical lab thresholds into one 'adequate hematologic/hepatic/renal function' "
    "item. Return STRICT JSON only:\n"
    "{\n"
    '  "items": [\n'
...
_EXPLAIN_SYSTEM = (
    ...
    "VOICE: speak DIRECTLY TO the patient in the second person — 'you', 'your tumor', "
    "'your age' — as if handing them this note. NEVER refer to them in the third person "
    "('the patient', 'them', 'this patient'). Use 'your care team' / 'your doctor', not "
    "'the care team'. (Exception: 'questions_to_ask' are the patient's OWN questions, so "
    "phrase those in the first person — 'I', 'my', 'me'.)\n"

...
- **Drug-name normalization** *(✅ DONE — both tiers built)*: `backend/app/drugs.py`. **Claude names** the drug mention → **Tier 1 RxNorm/RxNav** (key-free) resolves RxCUI + canonical ingredient (exact then approximate match; Temodar/TMZ → temozolomide) → **Tier 2 ChEMBL** adds mechanism of action / class (bevacizumab → "VEGF-A inhibitor" = grounds "prior anti-VEGF"). Cached in Postgres (`drug_normalizations`). Endpoints `POST /api/drugs/normalize` + `/api/drugs/from_patient`; UI card after Analyze with RxCUI/ChEMBL links. Reinforces "Claude is not the source of truth." **ChEMBL mechanism is now wired into the Stage-1 pre-screen** (`prescreen.drug_signals`): a trial excluding a drug *class* ("prior anti-VEGF therapy") — not the drug name — is flagged via mechanism (bevacizumab→VEGF-A inhibitor). For Case 004 this catches 4 real GBM trials a name-only filter misses; UI shows a "ChEMBL mechanism-matched N trials" callout. PRD §12.2.
...
>   Generated by **Claude Science** (literature map), then **every PMID verified against PubMed
>   E-utilities: 23/23 real, 0 hallucinated** (3 epub/print years corrected). Files:
>   `backend/app/evidence.json` + `evidence.py` + `GET /api/evidence`; frontend `EvidencePanel`
>   (collapsed, low-key traceability under the fit table — sources next to claims, PMID links,
>   NOT an explainer). Prompt archived: `docs/evidence-layer-prompt.md`.
> - **verify-agent guardrail** — symmetric honesty: as we never fake a *catch*, verify may never
>   fake a *doubt*. `_enforce_evidence_grounding()` reverts any overstated/unsupported flag whose
>   cited line isn't a real unknown/not_met criterion → back to `supported`. (`backend/app/main.py`)
> - **Positioning research** (verified via web): NBTS finder = **Ancora.ai** white-label (PRD §4.1.1,
>   rewritten). Drug-class bridging is NOT novel — **GenomOncology** does it; **MatchMiner/Tempus/
>   GenomOncology** are hospital-deployed genomic matchers; **Triomics** ($22M, MSK/Yale) does
>   per-criterion eligibility + citations. → Do NOT claim novelty on matching/eligibility. Our lane =
>   **after-the-match verify + patient-side shared decision + glioma WHO-CNS5 depth**. Healthcare LLMs
>   deployed today cluster in *documentation* (ambient scribes); our anti-hallucination stack
>   (grounding + citations + deterministic check + human-in-loop) mirrors best practice.
> - **Vercel fix** — live site showed "0 patients": build had no `NEXT_PUBLIC_API_URL` → baked the
>   `127.0.0.1:8000` fallback. Backend + CORS verified healthy. Committed **`frontend/.env.production`**
>   (= Render URL, force-added; `.env.*` gitignored). **VERIFY on boot: after Vercel rebuild the panel
>   shows 8 patients.** If still 0 → a wrong Vercel dashboard env var is overriding; fix there + redeploy.
> - **Claude Science case-provenance QC** (not yet wired to app): ran the `docs/case-provenance-prompt.md`
>   dataset QC; **independently re-verified against cBioPortal/GDC — our 8-case data is faithful**
>   (case-003 TCGA-CS-5396 is a *real* atypical codeleted-oligo with TP53 R273H, not our error;
>   case-002 correctly reclassifies GBM→Astrocytoma). Claude Science also generated 3 figures
>   (oncoprint / reclassification map / provenance breakdown) — **still to download + QC + wire in.**
>
> **Validation plan (decided):** primary reviewer = **Tianwei Long** (user's 哥哥 — PhD cand. Public
> Health, Atlantic Fellow, 10+ yr pharma, immediate family of a Grade-4 Astrocytoma patient → covers
> most clinical/trial correctness + patient side). Ask him to flag anything needing a treating clinician
> + to intro a neuro-oncologist if so (drafted messages exist). Clinician = optional bonus (UCSF via
> Gladstone; Stanford fellows via coordinator amysingh@stanford.edu). Honesty: describe his role
> accurately, get consent before quoting; NOT "physician-validated".
>
> **NEXT (Day 6, in priority):** ① verify Vercel now shows 8 patients (warm Render `/health` first);
> ② send Tianwei the review message; ③ write the 100–200w submission summary (title candidate:
> **"Does this trial fit *this* patient?"** — a glioma evidence-review & shared-decision copilot);
> ④ download + QC Claude Science's 3 figures, wire into app/demo; ⑤ record 3-min video
> (`docs/demo_script.md`); ⑥ `gh repo edit tangxiya-star/glioma-copilot --visibility public` (still
> PRIVATE); ⑦ submit before **Jul 13 9pm ET**. Deploy note: frontend auto-deploys from `main`; Render
> free tier sleeps (~30-50s cold start) — warm before any demo.
